...
NSString * Mobile_Web_App$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation Mobile_Web_AppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mobile_web_app);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Mobile_Web_App global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Mobile_Web_App" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mobile_web_app = [[Mobile_Web_AppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mobile_web_app];
	
	NSString *_mobile_web_appNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mobile_web_appNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mobile_web_app valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Mobile_Web_App$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mobile_web_app.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.mobile_web_app.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
